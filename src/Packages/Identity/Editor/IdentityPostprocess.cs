...
namespace Portal.Identity.Editor
{
    internal class IdentityPostprocess : IPostprocessBuildWithReport
    {
        ...
        {
            Debug.Log($"Identity post-processing...");
            ...
            // Copy identity files to data directory for these target
            ...
            if (buildTarget == BuildTarget.StandaloneWindows64 || buildTarget == BuildTarget.StandaloneOSX || buildTarget == BuildTarget.iOS)
            {
                ...
                Debug.Log($"Successfully copied Identity files");
            }
        ...
        private void CopyIntoDataDir(string buildDataPath)
        {
            ...
            // Identity folder in the data folder
            string buildIdentityPath = $"{buildDataPath}/PortalSDK/Runtime/Identity/";
        ...
        {
            Debug.Log("Copying Identity files...");
            ...
            foreach (string dir in Directory.GetDirectories(identityWebFilesDir, "*", SearchOption.AllDirectories))
            {
                string dirToCreate = dir.Replace(identityWebFilesDir, destinationPath);
                Directory.CreateDirectory(dirToCreate);
            }

            foreach (string newPath in Directory.GetFiles(identityWebFilesDir, "*.*", SearchOption.AllDirectories))
            {
                if (!newPath.EndsWith(".meta"))
                {
                    File.Copy(newPath, newPath.Replace(identityWebFilesDir, destinationPath), true);
                }
            }
        }
    }
