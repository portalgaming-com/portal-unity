...
namespace Portal.Browser.Gree
{
    public class GreeBrowserClient : IWebBrowserClient
    {
        ...
        private const string MAC_EDITOR_RESOURCES_DIRECTORY = "Packages/com.portal.identity/Runtime/Resources";
        ...
        public GreeBrowserClient()
        {
            ...
            string filePath = Constants.SCHEME_FILE + ANDROID_DATA_DIRECTORY + Constants.IDENTITY_DATA_DIRECTORY_NAME + Constants.IDENTITY_HTML_FILE_NAME;
#elif UNITY_EDITOR_OSX
            string filePath = Constants.SCHEME_FILE + Path.GetFullPath(MAC_EDITOR_RESOURCES_DIRECTORY) + Constants.IDENTITY_HTML_FILE_NAME;
#elif UNITY_STANDALONE_OSX
            string filePath = Constants.SCHEME_FILE + Path.GetFullPath(Application.dataPath) + MAC_DATA_DIRECTORY + Constants.IDENTITY_DATA_DIRECTORY_NAME + Constants.IDENTITY_HTML_FILE_NAME;
            filePath = filePath.Replace(" ", "%20");
#elif UNITY_IPHONE
            string filePath = Path.GetFullPath(Application.dataPath) + Constants.IDENTITY_DATA_DIRECTORY_NAME + Constants.IDENTITY_HTML_FILE_NAME;
#else
            string filePath = Constants.SCHEME_FILE + Path.GetFullPath(Application.dataPath) + Constants.IDENTITY_DATA_DIRECTORY_NAME + Constants.IDENTITY_HTML_FILE_NAME;
#endif
