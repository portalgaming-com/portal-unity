...
namespace VoltstroStudios.UnityWebBrowser.Core
{
    ...
    public class WebBrowserClient : IWebBrowserClient, IDisposable
    {
        ...
        public async UniTask Init(int engineStartupTimeout = 30000)
        {
            ...
            string filePath = "";
#if UNITY_EDITOR
            filePath = Constants.SCHEME_FILE + Path.GetFullPath($"{IDENTITY_PACKAGE_RESOURCES_DIRECTORY}{Constants.IDENTITY_HTML_FILE_NAME}");
#else
#if UNITY_STANDALONE_WIN || (UNITY_ANDROID && UNITY_EDITOR_WIN) || (UNITY_IPHONE && UNITY_EDITOR_WIN)
            filePath = Constants.SCHEME_FILE + Path.GetFullPath(Application.dataPath) + Constants.IDENTITY_DATA_DIRECTORY_NAME + Constants.IDENTITY_HTML_FILE_NAME;
#endif
#endif
            initialUrl = filePath;
